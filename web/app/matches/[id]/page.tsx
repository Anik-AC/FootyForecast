...
export default async function MatchDetailPage({ params }: PageProps) {
  ...
  const wentToET = result?.went_to_et ?? false;
  const wentToPens = result?.went_to_pens ?? false;
  const penWinnerID = result?.pen_winner_id ?? null;
  const resultStatusLabel = wentToPens ? "PENALTIES" : wentToET ? "EXTRA TIME" : "FULL TIME";
  const resultStatusColor = wentToPens ? "#FFC23D" : wentToET ? "#5B8CFF" : "#2BE38A";

  const homeWon = isCompleted && (
    wentToPens ? penWinnerID === home_team.id : result!.home_goals > result!.away_goals
  );
  const awayWon = isCompleted && (
    wentToPens ? penWinnerID === away_team.id : result!.away_goals > result!.home_goals
  );
    ...
    <div style={{ animation: "ff-up 0.4s ease both", paddingTop: 32, maxWidth: 740, margin: "0 auto" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
              {isCompleted ? (
                <>
                  ...
                  <span style={{ fontFamily: MONO, fontSize: 11, fontWeight: 700, color: resultStatusColor, letterSpacing: "0.12em" }}>
                    {resultStatusLabel}
                  </span>
                </>
              ) : (
      ...
      {!isCompleted && (
        <>
          ...
          {preview && (
            <div style={{
              ...
              <div style={{ padding: "24px 26px" }}>
                <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.12em", color: "#FFC23D", marginBottom: 10 }}>
                  MATCH PREVIEW
                </div>
                <div style={{ fontSize: 15, lineHeight: 1.65, color: "#C8C3D6" }}>{preview.preview_text}</div>
                <div style={{ fontSize: 12, color: "#645F77", marginTop: 14 }}>Generated by the FootyForecast model.</div>
              </div>
            </div>
          )}

          {/* Recent form + H2H in 2-col */}
          {(homeForm.length > 0 || awayForm.length > 0 || h2h) && (
            <div className="ff-grid-2col" style={{ marginTop: 16 }}>
              {(homeForm.length > 0 || awayForm.length > 0) && (
                <div style={{ background: "#15131F", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 16, padding: 24 }}>
                  <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.12em", color: "#C8C3D6", marginBottom: 18 }}>RECENT FORM</div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                    {homeForm.length > 0 && <TeamForm teamID={home_team.id} teamName={home_team.name} matches={homeForm} />}
                    {awayForm.length > 0 && <TeamForm teamID={away_team.id} teamName={away_team.name} matches={awayForm} />}
                  </div>
                </div>
              )}
              {h2h && <HeadToHead data={h2h} homeTeamName={home_team.name} awayTeamName={away_team.name} />}
            </div>
          )}

          {/* Forecast + expected goals in 2-col */}
          <div className="ff-grid-2col" style={{ marginTop: 16 }}>
            <ForecastCard prediction={prediction} scorers={scorers} />
            <div style={{ background: "#15131F", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 16, padding: 24 }}>
              <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.12em", color: "#C8C3D6", marginBottom: 18 }}>EXPECTED GOALS</div>
              {expected_goals ? (
                <div style={{ display: "flex", flexDirection: "column", gap: 22, textAlign: "center", marginTop: 4 }}>
                  <div>
                    <div style={{ fontFamily: MONO, fontSize: 40, fontWeight: 700, color: "#2BE38A", lineHeight: 1 }}>
                      {expected_goals.home_xg.toFixed(2)}
                    </div>
                    <div style={{ fontSize: 13, color: "#9E99B0", marginTop: 6 }}>{home_team.name} xG</div>
                  </div>
                  <div>
                    <div style={{ fontFamily: MONO, fontSize: 40, fontWeight: 700, color: "#5B8CFF", lineHeight: 1 }}>
                      {expected_goals.away_xg.toFixed(2)}
                    </div>
                    <div style={{ fontSize: 13, color: "#9E99B0", marginTop: 6 }}>{away_team.name} xG</div>
                  </div>
                </div>
              ) : (
                <p style={{ fontSize: 13, color: "#4A4560", margin: 0 }}>Not yet available.</p>
              )}
            </div>
          </div>

          {/* Over/under + top scorelines in 2-col */}
          <div className="ff-grid-2col" style={{ marginTop: 16 }}>
            <div style={{ background: "#15131F", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 16, padding: 24 }}>
              <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.12em", color: "#C8C3D6", marginBottom: 18 }}>OVER / UNDER</div>
              <OverUnderBars totals={totals} showBTTS />
            </div>
            <div style={{ background: "#15131F", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 16, padding: 24 }}>
              <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.12em", color: "#C8C3D6", marginBottom: 18 }}>TOP SCORELINES</div>
              {scoreline_grid && scoreline_grid.length > 0 ? (
                <TopScorelines grid={scoreline_grid} homeTeam={home_team.name} awayTeam={away_team.name} />
              ) : (
                <p style={{ fontSize: 13, color: "#4A4560", margin: 0 }}>Scoreline data not available.</p>
              )}
            </div>
          </div>
          ...
          {/* Goal threats */}
          {scorers && (
            <div style={{ background: "#15131F", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 16, padding: 24, marginTop: 16 }}>
              <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.12em", color: "#C8C3D6", marginBottom: 18 }}>
                GOAL THREATS <span style={{ color: "#645F77", fontWeight: 500, fontSize: 11 }}>· anytime scorer probability</span>
              </div>
              <PlayerScorers data={scorers} />
            </div>
          )}
          ...
          {hasMarketData && (
            <div style={{ background: "#15131F", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 16, padding: 24, marginTop: 16 }}>
              <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.12em", color: "#C8C3D6", marginBottom: 18 }}>MODEL VS MARKET</div>
              <MarketPanel data={market!} />
            </div>
          )}
          ...
          {trivia && trivia.facts.length > 0 && (
            <div style={{ background: "#15131F", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 16, padding: 24, marginTop: 16 }}>
              <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.12em", color: "#C8C3D6", marginBottom: 18 }}>PRE-MATCH FACTS</div>
              <TriviaFacts facts={trivia.facts} />
            </div>
          )}
        </>
